...
def iter_step(tmp_noise, origin_img, target_img, gaussian_blur, model_pool, index, loss1_v, momentum, lr, eps1, eps2,
    ...
    for model_dict in model_pool:
        ...
        loss1 += (tmp1 + tmp2) * proportion  # (tmp1 - tmp2) * proportion  # 只采用非定向，不使用定向，集成模型
    ...
    L1_norm_loss1_v = now_loss1_v

    loss1_v = L1_norm_loss1_v + loss1_v * momentum  # 使用动量项，利用loss1更新扰动
    # print(torch.max(tmp_noise.grad))
    loss1_v_gauss = F.conv2d(loss1_v, gaussian_blur, padding=(3 - 1) // 2, groups=3)  # 动量可以试着加在平滑后面

    grad_norms = loss1_v_gauss.view(1, -1).norm(p=float('inf'), dim=1)  # 使用L无穷进行正则化
    norm_loss1_v = loss1_v_gauss.div(grad_norms.view(-1, 1, 1, 1))
...
def noise_iter(model_pool, origin_img, target_img, gaussian_blur, device, break_threshold, flag,origin_path):
    learning_rate = maxeps / 255 * 2 / 20  # 可以调整,步长为1，因为图片放缩了，步长也要放缩
    ...
    while True:
        ...
        tmp_noise, score1, score2, loss1_v = iter_step(last_tmp_noise, origin_img, target_img, gaussian_blur, model_pool,
                                                       index, loss1_v, momentum, learning_rate, eps1, eps2, eps3)

        last_tmp_noise = last_tmp_noise.detach()
        #print(torch.max(last_tmp_noise),torch.max(tmp_noise),torch.mean(last_tmp_noise),torch.mean(tmp_noise))
        if score1 < break_threshold and flag != times:
            print(origin_path,index)

            noise = last_tmp_noise[0].cpu().numpy()
            noise = noise.swapaxes(0, 1).swapaxes(1, 2)

            """
            if (flag == 0):
                tmp_noise_save = last_tmp_noise  # 存储第一次噪声
            value = 1
            eps1 = np.quantile(np.abs(noise[:, :, 0]), value)  # value=0.95 273.15 453.3877000212669 544.32 75%
            eps2 = np.quantile(np.abs(noise[:, :, 1]), value)
            eps3 = np.quantile(np.abs(noise[:, :, 2]), value)
            print(flag, eps1, eps2, eps3, score1)
            """
            tmp_noise_save = last_tmp_noise.clone()  # 存储第一次噪声
            value = 1 - (flag + 1)*kd #保持0.97，一共五次下降
            eps1 = np.quantile(np.abs(noise[:, :, 0]), value)  # value=0.95 273.15 453.3877000212669 544.32 75%
            eps2 = np.quantile(np.abs(noise[:, :, 1]), value)
            eps3 = np.quantile(np.abs(noise[:, :, 2]), value)
            #print(flag,eps1,eps2,eps3,score1)
            last_tmp_noise[0][0] = last_tmp_noise[0][0].clamp_(-eps1, eps1)  # clip
            last_tmp_noise[0][1] = last_tmp_noise[0][1].clamp_(-eps2, eps2)  # clip
            last_tmp_noise[0][2] = last_tmp_noise[0][2].clamp_(-eps3, eps3)  # clip

            last_tmp_noise = (last_tmp_noise + origin_img).clamp_(-1, 1) - origin_img
...
def cal_adv(origin_path, target_path, model_pool, gaussian_blur, device, break_threshold):
    ...
    while True:
        ...
        if i >= max_iterations:  #总迭代次数不能超过300代，不然使用上次的攻击代数
            ...
            if flag ==0 :
                f = open('hard.txt', 'a')
                f.write(origin_path + ';' + target_path + ';' + str(scores) + '\n')
                f.close()
                print('origin img is %s, target img is %s, iter %d, socre is %0.3f,困难样本，攻击不了'
                      % (origin_path.split('/')[-1], target_path.split('/')[-1], i, scores))

            else:
                print('origin img is %s, target img is %s, iter %d, 使用上一次攻击得到图片'
                      % (origin_path.split('/')[-1], target_path.split('/')[-1], i, ))
                last_tmp_noise = tmp_noise_save  # 不使用clip后的噪声，使用原来eps为20满足条件的噪声


            """
            tmp_noise = get_init_noise(device, origin_img)
            """
            break
...
def attack(face_path, origin_name, tar_path, model_pool, gaussian_blur, device, break_threshold, tatget):
    ...
    noise = noise.resize((h, w), Image.NEAREST)  #resize噪声
    ...
    # adv_sample = adv_sample.resize((h, w), Image.NEAREST)  # 其实这里还要加一步，控制resize后的像素值是否溢出
    ...
    """
    # 保存图像
    output_dir = '../result_data/adv_images' + str(tatget) +'/'
...
def one_process_run(people_list, model_pool, device, face_path, tatget):
    ...
    for origin_name in people_list:
        ...
        if origin_name.split('_')[1] == '0.jpg':
            tar_name = origin_name.split('_')[0] + '_1.jpg'  # 自己类的第二张图片
        else:
