...
def iter_step(tmp_noise, origin_img, target_img, gaussian_blur, model_pool, index, loss1_v, momentum, lr, eps1, eps2,
    ...
    if (grad_norms == 0).any():  # avoid nan or inf if gradient is 0
        norm_loss1_v = torch.randn_like(grad_norms)
    """
    tmp_noise = tmp_noise.detach() - lr * torch.sign(loss1_v)  # 不用正则化
...
def cal_adv(origin_path, target_path, model_pool, gaussian_blur, device, break_threshold):
    ...
    while True:
        ...
        if score1 < break_threshold and flag == times :#and i > 20:
